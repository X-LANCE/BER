...
def main():
    ...
    for filename in rttm_dict.keys():
        ...
        bder_dict[filename] = dict()
        bder_dict[filename]['optimal_mapping'] = dict()
        bder_dict[filename]['optimal_mapping']['r2h'] = dict() # ref to hyp speaker mapping after optimal_mapping
        bder_dict[filename]['optimal_mapping']['h2r'] = dict() # hyp to ref
        bder_dict[filename]['optimal_mapping']['r_ms_id'] = set() # not matched ref id, also called missed speaker
        bder_dict[filename]['optimal_mapping']['h_ms_id'] = set() # not matched hyp id, also called false alarm speaker
        bder_dict[filename]['optimal_mapping']['r_ms_case'] = []  # not matched ref case
        bder_dict[filename]['optimal_mapping']['h_ms_case'] = []  # not matched hyp case
        
        bder_dict[filename]['ser'] = dict()
        bder_dict[filename]['ser']['ref'] = dict()
        bder_dict[filename]['ser']['hyp'] = dict()
        
        bder_dict[filename]['jer'] = dict()
        bder_dict[filename]['jer']['ref'] = dict()
        bder_dict[filename]['jer']['hyp'] = dict()
        
        ref = rttm_dict[filename]['ref']
        if 'hyp' not in rttm_dict[filename].keys(): # no predictions for current file
            hyp = []
        else:
            hyp = rttm_dict[filename]['hyp']
        ...
        for spk_name in ref_set:
            ...
            bder_dict[filename]['ser']['ref'][spk_name] = dict()
            bder_dict[filename]['ser']['ref'][spk_name]['ms_num'] = 0
            bder_dict[filename]['ser']['ref'][spk_name]['total_num'] = len(ref_dict[spk_name])
            bder_dict[filename]['ser']['ref'][spk_name]['ms_case'] = []
            bder_dict[filename]['ser']['ref'][spk_name]['ms_reason'] = []
            bder_dict[filename]['ser']['ref'][spk_name]['iou_threshold'] = []
            
            # init for duration-level
            bder_dict[filename]['jer']['ref'][spk_name] = dict()
            bder_dict[filename]['jer']['ref'][spk_name]['fa_duration'] = 0
            bder_dict[filename]['jer']['ref'][spk_name]['ms_duration'] = 0
            bder_dict[filename]['jer']['ref'][spk_name]['ref_duration'] = sum([ end - start for start,end in ref_dict[spk_name]])
            bder_dict[filename]['jer']['ref'][spk_name]['union_duration'] = 0
        ...
        for ref_id, hyp_id in zip(row_index, col_index): # matched result
            ...
            bder_dict[filename]['optimal_mapping']['r2h'][index_ref[ref_id]] = index_hyp[hyp_id]
            bder_dict[filename]['optimal_mapping']['h2r'][index_hyp[hyp_id]] = index_ref[ref_id]
        ...
        for ref_name in notmatched_ref_set:
            ...
            bder_dict[filename]['ser']['ref'][ref_name]['ms_num'] = len(ref_dict[ref_name])
            bder_dict[filename]['ser']['ref'][ref_name]['total_num'] = len(ref_dict[ref_name])
            
            bder_dict[filename]['jer']['ref'][ref_name] = dict()
            bder_dict[filename]['jer']['ref'][ref_name]['fa_duration'] = 0 
            bder_dict[filename]['jer']['ref'][ref_name]['ms_duration'] = 0 
            bder_dict[filename]['jer']['ref'][ref_name]['ref_duration'] = 0 
            bder_dict[filename]['jer']['ref'][ref_name]['union_duration'] = 0
            
            for start, end in ref_dict[ref_name]: 

                # all segments are regarded as errors
                bder_dict[filename]['optimal_mapping']['r_ms_case'].append((start, end))
                bder_dict[filename]['ser']['ref'][ref_name]['ms_case'].append((start, end))
                bder_dict[filename]['ser']['ref'][ref_name]['ms_reason'].append('optimalmapping')
                bder_dict[filename]['ser']['ref'][ref_name]['iou_threshold'].append(['na'])
                
                # all segments are regarded as missed speech
                bder_dict[filename]['jer']['ref'][ref_name]['ms_duration'] += end-start
        ...
        for hyp_name in unmatched_hyp_set: # unmatched ref result (false alarm speakers)
            ...
            bder_dict[filename]['ser']['hyp'][hyp_name] = dict()
            bder_dict[filename]['ser']['hyp'][hyp_name]['ms_num'] = len(hyp_dict[hyp_name])
            bder_dict[filename]['ser']['hyp'][hyp_name]['total_num'] = len(hyp_dict[hyp_name])
            bder_dict[filename]['ser']['hyp'][hyp_name]['ms_case'] = []
            bder_dict[filename]['ser']['hyp'][hyp_name]['ms_reason'] = []
            bder_dict[filename]['ser']['hyp'][hyp_name]['iou_threshold'] = []
            
            bder_dict[filename]['jer']['hyp'][hyp_name] = dict()
            bder_dict[filename]['jer']['hyp'][hyp_name]['fa_duration'] = 0 
            bder_dict[filename]['jer']['hyp'][hyp_name]['ms_duration'] = 0 
            bder_dict[filename]['jer']['hyp'][hyp_name]['ref_duration'] = 0 
            bder_dict[filename]['jer']['hyp'][hyp_name]['union_duration'] = 0 
                
            
            for start, end in hyp_dict[hyp_name]:
                # all segments are regarded as errors
                bder_dict[filename]['optimal_mapping']['h_ms_case'].append((start, end))

                bder_dict[filename]['ser']['hyp'][hyp_name]['ms_case'].append((start, end))
                bder_dict[filename]['ser']['hyp'][hyp_name]['ms_reason'].append('optimalmapping')
                bder_dict[filename]['ser']['hyp'][hyp_name]['iou_threshold'].append(['na'])
                
                # all segments are regarded as missed speech
                bder_dict[filename]['jer']['hyp'][hyp_name]['ms_duration'] += end-start
        ...
        for spk_name in matched_hyp_set:
            bder_dict[filename]['ser']['hyp'][spk_name] = dict()
            bder_dict[filename]['ser']['hyp'][spk_name]['ms_num'] = 0
            bder_dict[filename]['ser']['hyp'][spk_name]['total_num'] = len(hyp_dict[spk_name])
            bder_dict[filename]['ser']['hyp'][spk_name]['ms_case'] = []
            bder_dict[filename]['ser']['hyp'][spk_name]['ms_reason'] = []
            bder_dict[filename]['ser']['hyp'][spk_name]['iou_threshold'] = []
        ...
        for ref_id, hyp_id in zip(row_index, col_index):
            ...
            bder_dict[filename]['jer']['ref'][ref_name]['fa_duration'] = fa_duration
            bder_dict[filename]['jer']['ref'][ref_name]['ms_duration'] = ms_duration
            bder_dict[filename]['jer']['ref'][ref_name]['union_duration'] = union_duration
            bder_dict[filename]['jer']['ref'][ref_name]['ref_duration'] = ref_duration
            ...
            for p, single_pair in enumerate(total_pair):
                ...
                if iou < iou_adapt:
                    ...
                    for start, end in single_ref_segs:
                        ...
                        bder_dict[filename]['ser']['ref'][ref_name]['ms_num'] += 1 # marked as ref missed error
                        bder_dict[filename]['ser']['ref'][ref_name]['ms_case'].append((start,end))
                    ...
                    for start, end in single_hyp_segs:
                        bder_dict[filename]['ser']['hyp'][hyp_name]['ms_num'] += 1 # marked as hyp missed error, not used in our metric
                        bder_dict[filename]['ser']['hyp'][hyp_name]['ms_case'].append((start,end))
            ...
            for seg_id in unmatched_segs_ms_ref_set:
                ...
                bder_dict[filename]['ser']['ref'][ref_name]['ms_num'] += 1
                bder_dict[filename]['ser']['ref'][ref_name]['ms_case'].append((start,end))
            ...
            for seg_id in unmatched_segs_ms_hypo_set:
                ...
                bder_dict[filename]['ser']['hyp'][hyp_name]['ms_num'] += 1
                bder_dict[filename]['ser']['hyp'][hyp_name]['ms_case'].append((start,end))
